...
1. **进入邮件路由设置**：在Cloudflare账户主页中，点击已托管的域名，以我的域名为例，左侧导航栏选择“电子邮件路由”（Email），再点击“邮件路由”（Email Routing）。
   ...
   ![image-20251006172935227](image-20251006172935227.png)

2. **跳过新手引导**：首次进入该页面会弹出“新手设置”（Getting Started）弹窗，点击“取消”（Cancel）以进入自定义配置界面。

   ![image-20251006173116280](image-20251006173116280.png)
...
3. **启用邮件路由并添加DNS记录**：点击“启用电子邮件路由”，系统将自动生成4条用于邮件路由的DNS记录。点击“添加记录并启用Cloudflare”（Add Records & Enable Cloudflare），系统会自动完成这4条记录的添加，无需手动操作。
   ...
   ![image-20251006173157575](image-20251006173157575.png)
...
4. **配置“全部接收”（Catch All）规则**：
   ...
   - 在“操作”（Action）下拉菜单中选择“发送到电子邮件”，输入用于接收转发邮件的个人邮箱（如163邮箱），点击“保存”（Save）。
     ...
     ![image-20251006173414380](image-20251006173414380.png)
...
5. **验证接收邮箱**：保存后，如果是首次使用该邮箱，“Catch All”规则状态显示“待验证”（Pending Verification），Cloudflare会向填写的个人邮箱发送验证邮件。登录该邮箱，点击邮件中的蓝色按钮“验证电子邮件地址”（Verify Email Address）链接，完成验证。
   ...
   ![image-20251006173555698](image-20251006173555698.png)

   ![image-20251006173643620](image-20251006173643620.png)

6. **确认配置生效**：返回Cloudflare邮件路由页面并刷新，若“Catch All”规则旁显示对勾，即表示`gemsly.top`域名的邮件接收功能配置完成。

   ![image-20251006173720406](image-20251006173720406.png)
...
2. **创建Resend API Key**：
   ...
   - 自定义API Key名称可以随意写一个，下方两项设置保持默认，点击“添加”（Add）；
     ...
     ![image-20251006174429062](image-20251006174429062.png)

   - 生成API Key后，需立即复制保存，这个只会显示一次，随便写在一个地方。

     ![image-20251006174450100](image-20251006174450100.png)

3. **添加`gemsly.top`域名至Resend**：

   - 左侧导航栏选择“Domains”，点击“Add Domain”，以我的域名为例，输入你自己的域名，地区可以选一个离自己近的，点击“添加”；

     ![image-20251006174716346](image-20251006174716346.png)

   - 系统将生成3条用于邮件发送的DNS记录，记录每条记录的“类型”“名称”“值”及“优先级”（MX记录优先级为10）。用 Cloudflare 托管的域名是能一键设置的，点击登录 Cloudflare 就能一键设置。

     ![image-20251006174913213](image-20251006174913213.png)

     ![image-20251006175103955](image-20251006175103955.png)
...
5. **验证Resend域名配置**：返回Resend控制台的域名配置页面，点击“验证DNS记录”（Verify DNS Records），系统将检查Cloudflare中添加的记录是否正确。待3条记录均显示绿色“已验证”状态，即表示邮件发送的前置配置完成。
   ...
   ![image-20251006180229480](image-20251006180229480.png)
...
3. 设置一下在线 post header：

   ![image-20251006183244950](image-20251006183244950.png)

4. 以及请求的 body：

   ![image-20251006183317647](image-20251006183317647.png)

5. 执行命令并验证：在在线 post 中运行修改后的网页 api 和参数，若执行成功，返回200OK 和 id登录接收邮箱查看测试邮件。

   ![image-20251006183352708](image-20251006183352708.png)

   ![image-20251006183409845](image-20251006183409845.png)
...
![image-20251006183610524](image-20251006183610524.png)
